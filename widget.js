(function() {
  ...
  const styles = `
    ...
    .google-icon {
      ...
      height: 20px;
      opacity: 0.95;
      filter: drop-shadow(0 4px 10px rgba(17,24,39,0.12));
    }
    ...
    .avatar {
      ...
      margin-bottom: 12px;
      object-fit: cover;
      border: 2px solid rgba(236,72,153,0.65); /* Brand Pink Border */
    ...
    .nav-button {
      ...
      background: linear-gradient(145deg, #fff8fc 0%, #ffe7f1 55%, #ffffff 100%);
      color: #ec4899;
    ...
    .dot {
      ...
      height: 10px;
      border-radius: 50%;
      background-color: #f8cade; /* Soft pink */
      cursor: pointer;
      transition: all 0.3s ease;
    }
    .dot.active {
      background: linear-gradient(135deg, #ec4899, #f472b6);
      transform: scale(1.2);
      box-shadow: 0 0 0 4px rgba(236,72,153,0.14), 0 6px 14px rgba(236,72,153,0.22);
    }
  ...
  function startCarousel(shadow, totalSlides) {
    ...
    function slideAmountForIndex(index) {
      const gap = 20; // px
      return `calc((100% + ${gap}px) / ${itemsPerView} * ${index})`;
    }
    ...
    window.addEventListener('mousemove', (e) => {
      ...
      track.style.transform = `translateX(calc(-1 * ${slideAmountForIndex(currentIndex)} + ${dragDeltaX}px))`;
    });
    ...
    function endDesktopDrag() {
      ...
      if (Math.abs(dragDeltaX) > minDragDistance) {
        if (dragDeltaX < 0) {
          nextSlide();
        } else {
          prevSlide();
    ...
    window.addEventListener('mouseup', endDesktopDrag);
    track.addEventListener('mouseleave', endDesktopDrag);
    track.addEventListener('dragstart', (e) => e.preventDefault());

    // Start initial autoplay
    startAutoplay();
    // Autoplay continues while hovered
